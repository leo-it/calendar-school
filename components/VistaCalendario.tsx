...
function VistaSemana({
  ...
  if (vistaLandscape) {
    return (
      <>
        <button
          onClick={() => setVistaLandscape(false)}
          className="md:hidden fixed top-4 right-4 z-50 bg-primary-600 text-white p-3 rounded-full shadow-lg hover:bg-primary-700 transition-all duration-200 active:scale-95"
        ...
        <div className="md:hidden fixed inset-0 z-40 bg-white overflow-y-auto overflow-x-auto">
          <div className="min-h-screen pb-20" style={{ minWidth: '700px' }}>
            <div className="bg-gray-50 border-b px-4 py-3 sticky top-0 z-10" style={{ minWidth: '700px' }}>
            ...
            <div className="grid grid-cols-7 border-b" style={{ minWidth: '700px' }}>
              {dias.map((dia) => (
                <div
                  key={dia.toISOString()}
                  className={`p-3 text-center border-r last:border-r-0 min-w-[100px] ${
                ...
                >
                  ...
                  <div className="text-base mt-1 font-semibold text-gray-400">
                    {format(dia, 'd')}
                  </div>
                  {!mismoMes && (
            ...
            <div className="grid grid-cols-7 min-h-[500px]" style={{ minWidth: '700px' }}>
              {dias.map((dia) => {
                const clasesDelDia = clases.filter((clase) => {
                  if (!clase.fecha) return false
                  const fechaClase = parseFechaLocal(clase.fecha)
                ...
                return (
                  <div
                    key={dia.toISOString()}
                    className="border-r last:border-r-0 p-2 min-h-[500px] min-w-[100px]"
                  >
  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed right-4 md:right-6 bottom-20 z-50 bg-primary-600 text-white p-3 md:p-4 rounded-full shadow-lg hover:bg-primary-700 transition-all duration-200 active:scale-95"
      ...
      {/* Vista Desktop: Grid horizontal - Visible siempre en desktop */}
      ...
      <div className={`${vistaLandscape ? 'hidden' : 'block'} md:hidden bg-white rounded-lg shadow-sm`}>
        {dias.map((dia) => {
          const clasesDelDia = clases.filter((clase) => {
            if (!clase.fecha) return false
            // Normalizar ambas fechas a medianoche en hora local para comparar correctamente
